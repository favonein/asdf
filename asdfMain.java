...
public class asdfMain {
	final static ArrayList<String> finalSeq = new ArrayList<String>();

	public static String Assembler(ArrayList<String> seqs, int L) {
		// start by adding the first segment and build off of that
		for (int x = 0; x < L; x++)
			finalSeq.add(seqs.get(0).substring(x, x + 1));
		seqs.remove(0);
		System.out.println();
		boolean complete = false;
		// add to the end of first segment until finished
		while (!complete) {
			complete = true;
		...
		//
		//
		// add to the front of segment until finished
		complete = false;
		while (!complete) {
			complete = true;
			for (int x = seqs.size() - 1; x >= 0; x--) {
		...
		return finalSeqtoString(finalSeq);
	}

	// toString to format finalSeq into one string
	private static String finalSeqtoString(ArrayList<String> finalSeq) {
		String seq = "";
	...
	private static void removeAll(String remoo) {
		for (int x = finalSeq.size() - 1; x >= 0; x--) {
			if (finalSeq.get(x).equals(remoo))
				finalSeq.remove(x);
		}
	}
}
